...
sequence = foldRight (\x xs -> (:.) <$> x <*> xs) (pure Nil)
...
replicateA ::
  ...
  -> f (List a)
replicateA n f =
  sequence $ const f <$> range n
    where
      range :: Int -> List Int
      range n = if n == 0 then Nil else n :. range (n - 1)
...
filtering ::
  ...
  -> f (List a)
filtering p =
  foldRight (\x xs -> (\p' i e -> if p' then i else e) <$> p x <*> ((x:.) <$> xs) <*> xs) (pure Nil)
